...
def vitals_form():
    if request.method == "POST":
        ...
        # seperate_Sentences=line_seperator(bot_response)
        print("Bot Responce:***** ",bot_response)
        Remedy,Medicine = transform_diagnosticResponce(bot_response)
        bot_question = find_numbered_question_sentences(bot_response)    
        if len(Remedy)>=1 or len(Medicine)>=1:
             bot_response = ""
             bot_question = []
        
        return jsonify({'result': bot_response,'Questions': bot_question})
    else:
...
def transform_diagnosticResponce(bot_response_ai):
    # print("bot response",bot_response_ai)
...
def generate_question_with_langchain(data, question):
    ...
    if patient_data is not None:
        try: 
            ...
            You will ask question about patient health conditions.
            You will ask about symptoms and signs of various types of illnesses.
